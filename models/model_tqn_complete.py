# from AgentFormer import *
from AgentFormer2 import *
...
class ProcedureFormer(nn.Module):
    def __init__(
        ...
        decoder_layers = AgentFormerDecoderLayer(
            ...
            dropout=0.1, # 2nd dropout that I have in my network.
        )
        self.tf_decoder = AgentFormerDecoder(decoder_layers, num_layers=nlayer)
        ...
        # self.query_embed = torch.nn.Embedding(pred_horz - 1, d_model - noise_dim)
        ...
        self.discriminator1 = MLP(d_model - noise_dim, 32, [64, 32])
        # self.discriminator1 = MLP(64, 32, [96, 32])
    ...
    def inference(self, x, z, pred_horz=3, NDR=False):
        ...
        memory = self.keyvalue_embed.weight.unsqueeze(1)
        # Allow batch-operation on memories
        memory = memory.repeat(1, x.shape[0], 1)
