...
def main(_):
    ...
    input_path = os.path.join(NIV_PATH, "videos/")
    input_path_labels =  os.path.join(NIV_PATH, "csvs/")

    # read through annotation folder
    annt_folder = os.path.join(NIV_PATH, "csvs")
    ...
    in_dir = glob.glob(input_path + "*.mpg")  # all mp4 files to be processed
    exit_vids = glob.glob(
        os.path.join(NIV_PATH, "processed_data/" + "*.npy")
    )

    with open(os.path.join(NIV_PATH, 'full_action_ids.pickle'), 'rb') as f:
        action2id = pickle.load(f)
    ...
    tobe_proc_id_list = []
    for video in in_dir:
        vid_name = video.replace("mpg", "csv").split('/')[-1]
        activity_name = vid_name.split('_')[0]
        if vid_name in split_annt_files_names:
            ...
            npy_file = os.path.join(
                os.path.join(NIV_PATH, "processed_data"),
                vid_name.replace("csv", "npy"),
            )
    ...
    # task = read_task_files(annt_files)
    # data = {**task}

    "Specify Location to output the tfrecord file"
    tfrecord_path = os.path.join(NIV_PATH, "niv.tfrecord")
            ...
            try:
                ...
                if os.path.exists(video_labels_path):
                    ...
                    for i in range(num_steps):
                        ...
                        duration = num_steps
                        steps = annotation[i][0]
                        task_id = action2id[activity_name+annotation[i][0]]
