...
def encode_numpy(tf_dataset, dataset, dest, write_frequency=1):

    import pickle

    print("Generate numpy files to %s" % dest)
...
def encode_folder(source, dest, dataset_name, val=True):
    ...
    for tfrecord in tfrecords:
        ...
        encode_numpy(tf_dataset=dataset, dataset=dataset_name, dest=dest)
