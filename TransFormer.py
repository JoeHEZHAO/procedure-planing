...
def attention(
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            if in_proj_weight_self is not None:
                q_self, k_self = linear(
                    query, in_proj_weight_self, in_proj_bias_self
                ).chunk(2, dim=-1)
    ...
    if in_proj_weight_self is not None:
        q_self = (
            q_self.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        )
    ...
    if in_proj_weight_self is not None:
        ...
        attn_weight_self_mask = attn_weight_self_mask.repeat(
            [
                attn_output_weights.shape[1] // num_agent,
                attn_output_weights.shape[2] // num_agent,
            ]
        ).unsqueeze(0)
...
class TransFormerDecoder(Module):
    ...
    def forward(
        self,
        tgt: Tensor,
        k_memory: Optional[List] = None,
        v_memory: Optional[List] = None,
        ...
        """Size of memory does not affect the size of output?? """

        """So,
        I need to figure out three things:
            1). How to do the encoding?
            2). How to do the autoregressive decoding:
                a). What is the first start?
                b). what is the output ?
                c). How to reuse the output?
            3). What is the final return? All middle predictions, or just the last one?
            4). How to do the positional encoding?
        """
        return output, {
...
class TransFormerDecoderLayer(Module):
    ...
    def __init__(
        self, cfg, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"
    ):
        super().__init__()
        self.cfg = cfg
        self.self_attn = Attention(cfg, d_model, nhead, dropout=dropout)
        self.multihead_attn = Attention(cfg, d_model, nhead, dropout=dropout)
    ...
    def forward(
        ...
        Args:
            tgt: the sequence to the decoder layer (required), shape [seq_len, batch, feat_dim]
            memory: the sequence from the last layer of the encoder (required).
            tgt_mask: the mask for the tgt sequence (optional).
            memory_mask: the mask for the memory sequence (optional).
            tgt_key_padding_mask: the mask for the tgt keys per batch (optional).
...
if __name__ == "__main__":
    ...
    test_inp = torch.rand(1, 1, 128)
    num_a = 1
    test_posenc_inp = pos_encoder(test_inp, num_a)


    """

    """Test the decoder Formder function;
    decoder_layers = TransFormerDecoderLayer(
        cfg={}, d_model=128, nhead=8, dim_feedforward=512, dropout=0.1
    )
    tf_decoder = TransFormerDecoder(decoder_layers, num_layers=2)

    test_inp = torch.rand(1, 1, 128)
    context_inp = torch.rand(12, 1, 128)
    mem_mask = None
    tgt_mask = None
    tf_out, _ = tf_decoder(
        test_inp, context_inp, memory_mask=mem_mask, tgt_mask=tgt_mask, num_agent=1
    )
    print(tf_out.shape)

    """

    ## Encode the [start, goal] for context
    encoder_layers = TransFormerEncoderLayer(
        cfg={}, d_model=128, nhead=8, dim_feedforward=512, dropout=0.1
    )
    tf_encoder = TransFormerEncoder(encoder_layers, num_layers=2)

    test_inp = torch.rand(2, 1, 128)
    test_start_pos = pos_encoder(test_inp[0:1], num_a=1, t_offset=0)
    test_goal_pos = pos_encoder(test_inp[1:], num_a=1, t_offset=3)
    test_inp = torch.cat([test_start_pos, test_goal_pos], 0)

    mask = torch.zeros((test_inp.shape[0], test_inp.shape[0]))
    context_output = tf_encoder(test_inp, mask=mask, num_agent=1)
    print(context_output.shape)

    ## Decode the [start, goal] for context
    decoder_layers = TransFormerDecoderLayer(
        cfg={}, d_model=128, nhead=8, dim_feedforward=512, dropout=0.1
    )
    tf_decoder = TransFormerDecoder(decoder_layers, num_layers=2)

    cls_decoder = MLP(128, 106, [256, 256])
    cls_encoder = MLP(106, 128, [256, 256])
    ...
    for i in range(3):
        if i == 0:
            out, _ = tf_decoder(test_start_pos, context_output)
            # seq_out = torch.cat([test_start_pos, out], 0)
        else:
            out, _ = tf_decoder(input, context_output)

        # Re-encode the position prior to newly predicted result
        out = pos_encoder(out, num_a=1)
        out = torch.cat([test_start_pos, out], 0)

        seq_out = cls_decoder(out)  # perform gumbel-sampling if training

        if i < pred_horz:
            input = cls_encoder(seq_out)
