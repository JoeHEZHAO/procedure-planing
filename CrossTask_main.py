...
def train_generator(epoch):
    print("For epoch {}, start training the model-generator with adversarial learning".format(epoch))
    model.train()
    ...
    for batch in trainloader:
        ...
        for sample in batch:
            ...
            logits, state, noise_z = model(x, args.pred_horz)
            gt_state = model.state_encoder(x).mean(2)
        ...
        "Adv learning for Discriminator "
        # loss1 = 0.5 * sum(loss1) / len(loss1)
        # loss2 = sum(loss2) / len(loss2)
        ...
        loss.backward()
        optimizer_d.step()
    print("For batch {}, averaged loss {} of traning model-generator with adversaril learning".format(i, loss))


def train_discriminator(epoch):
    print("For epoch {}, start training the model-discriminator with adversarial learning".format(epoch))
    model.train()
    ...
    for batch in trainloader:
        ...
        for sample in batch:
            ...
            # output, gt, logits = model(torch.cat(x_input, 0))
            # output, gt, logits = model(x)
            # x = x[:, :-1]
            logits, state, noise_z = model(x, args.pred_horz)
            gt_state = model.state_encoder(x).mean(2)
        ...
        c_loss_v1 = contra_loss(pred_gt_sim, labels)
        c_loss_v2 = nce_loss(pred_gt_sim, pred_gt_sim_y, labels)
        # logits = model(torch.cat(x_input, 0), args.pred_horz).reshape(-1, act_size)
        # loss2 = ce_loss(
        # logits.squeeze(),
        # torch.cat(w_input, 0)[:, 1:].squeeze().reshape(-1, 1).squeeze(),
        # )

        # logsoftmax = nn.LogSoftmax(dim=1)
        # loss2.append(nll_loss(logsoftmax(logits.squeeze()), w[:, 1:].squeeze()))
        # loss2 = nll_loss(logsoftmax(logits.squeeze()), w[:, 1:].squeeze().reshape(-1, 1).squeeze())
        # loss1 = mse_loss(output, gt)
        # loss2 = ce_loss(logits.squeeze(), w[:, 1:].squeeze().reshape(-1, 1).squeeze())
        ...
        adv_loss = bce_loss(g_fake_logits, gt_real)
        ...
        # loss2.backward()
        optimizer.step()
        # print("For epoch {}, feature mse loss is {}".format(epoch, loss1.item()))
        # print("For epoch {}, Mse loss {}, CE  loss is {}".format(epoch, loss1, loss2))
    # print("For batch {}, finish traning".format(i))
    print("For batch {}, averaged loss {} of traning model-generator with adversaril learning".format(i, loss))


def train_NDR(epoch):
    print("For epoch {}, start training the generator with NDR loss")
    model.train()
    total_loss = []
    for batch in trainloader:
        optimizer.zero_grad()
        ...
        for sample in batch:
            ...
            state_pred_list_sample = []
            noise_z_list = []
            num_ndr_sample = 20
            # for i in range(num_ndr_sample):
            #     _, state, noise_z = model(x, args.pred_horz)
            ...
            replicate_x = x.repeat(num_ndr_sample, 1, 1, 1)
            logits, state_preds = model.inference(
                replicate_x, noise_z, args.pred_horz, NDR=True)
            model_logits = logits.clone().squeeze().cuda()

            # state_pred_samples = torch.stack(state_pred_list_sample).squeeze()
        ...
        "NDR training for the Generator "
        ndr_loss = sum(loss_ndr) / len(loss_ndr)
        scaling_factor = 1
        loss = scaling_factor * ndr_loss
        loss.backward()
    ...
    with torch.no_grad():
        for batch in testloader:
            for sample in batch:
                ...
                norm_logits = F.softmax(logits.squeeze(), dim=-1)
                ...
                path = viterbi_path(
                    ...
                    logits.squeeze().permute(1, 0)[1:].cpu().numpy(),
                )
                ...
                for _ in range(num_samples):
                    sample_word = model_logits.view(-1, 106).argmax(1)
                    tmp = sample_word.squeeze().cpu().numpy().tolist()
                ...
                """ Evaluate 10 or 1500 random samples """
                # ratio_list = []
                # num_samples = 100
                sample_list = []
                if criter is not None:
                    ...
                    # sample_list = []
                    for _ in range(num_samples):
                        sample_word = torch.multinomial(
                            F.softmax(model_logits.view(-1, 106), 1),
                            1,
                        )
                        sample_word = model_logits.view(-1, 106).argmax(-1)
                        sample_list.append(sample_word.cpu().numpy())

                # ratio_list.append(
                # (sample_word.squeeze().cpu().numpy() == dist_samples)
                # .all(1)
                # .any()
                # )
                # ratio = sum(ratio_list) / num_samples
                # mc_list.append(ratio)

                # all_samples = np.stack(sample_list).squeeze()
                # num_expert = dist_samples.shape[0]
                # list_expert = np.array_split(dist_samples, num_expert)
                # list_all_samples = np.array_split(all_samples, 1500)
                # tmp_recall = []
                # for item in list_expert:
                # tmp_recall.append((item == all_samples).all(1).any())
                # mc_recall.append(sum(tmp_recall) / len(tmp_recall))

                # Or only use the viterbi output
                # argmax_rst = model_logits.squeeze().argmax(1)
                # ratio_list.append(
                # (argmax_rst.squeeze().cpu().numpy() == dist_samples)
                # .all(1)
                # .any()
                # )
                ...
                if criter is not None:
                    ...
                    """Calcualate the nll w.r.t ref_dist, collected from all_ref (testest references) """
                    nll_tmp = []
                    for itm in sample_list:

                        norm_ref_dist = torch.nn.functional.log_softmax(
                            # norm_ref_dist = torch.nn.functional.softmax(
                            ref_dist, 1).cuda()
                        nll_tmp.append(
                            F.nll_loss(
                                norm_ref_dist,
                                # ref_dist,
                                torch.from_numpy(itm).squeeze().cuda(),
                            )
                        )
                    ...
                    ref_ce_rst = np.where(np.isnan(ref_ce_rst), 0, ref_ce_rst)
                    # ref_ce_rst = custom_kldiv(
                    # torch.from_numpy(ref_ce_rst), model_logits.cpu().squeeze()
                    # torch.from_numpy(ref_dist),
                    # ref_dist.cpu(),
                    # model_logits.cpu().squeeze(),
                    # )

                    ref_ce_list.append(ref_ce_rst)
